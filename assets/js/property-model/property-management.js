...
(function () {
        ...
        function (res) {
          ...
          if (res.success) {
            ...
            if (
              ...
              LEB_Toaster.show(
                'No listings found for "' + lebPmState.search + '"',
                "info",
              );
            }
          } else {
            renderCards([]);
            LEB_Toaster.show(
              (res.data && res.data.message) || "Failed to load listings.",
              "error",
            );
          }
        },
  ...
  function renderCards(items) {
    ...
    const editBase =
      lebPmState.editBaseUrl ||
      window.location.href.split("?")[0] +
        "?page=leb-properties&leb_action=edit&id=";

    DOM.body.innerHTML = items
      .map(function (item, index) {
        // Thumbnail - item.first_image is now a URL string from backend
        const placeholder =
          (LEB_Ajax.assets_url || "") + "images/placeholder.png";
        const thumb = item.first_image
          ? '<img class="leb-pm-card-thumb" src="' +
            escHtml(item.first_image) +
        ...
        return (
          '<div class="leb-pm-card" data-id="' +
          item.id +
          '">' +
          // Checkbox
          '<input class="leb-pm-card-checkbox leb-pm-row-check" type="checkbox" value="' +
          item.id +
          '">' +
          // Serial Number
          '<div class="leb-pm-card-sno">' +
          sNo +
          "</div>" +
          // Thumbnail
          '<div class="leb-pm-card-thumb-wrap">' +
          thumb +
          "</div>" +
          // Card Data Body
          ...
          "Edit" +
          "</a>" +
          '<button class="leb-pm-action-btn leb-pm-duplicate-btn" type="button" data-id="' +
          item.id +
          '">' +
          '<svg class="leb-pm-action-btn-icon" viewBox="0 0 24 24" fill="none" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>' +
          "Duplicate" +
          "</button>" +
          '<button class="leb-pm-action-btn leb-pm-action-btn--delete leb-pm-delete-btn" type="button" data-id="' +
          item.id +
          '">' +
  ...
  function renderPagination(data) {
    ...
    // Prev
    html +=
      '<button class="leb-pm-page-btn" data-page="' +
      (page - 1) +
      '"' +
      (page <= 1 ? " disabled" : "") +
      ">" +
    ...
    buildRange(page, totalPages).forEach(function (p) {
      if (p === "...") {
        html +=
          '<span class="leb-pm-page-btn" style="pointer-events:none;border:none;background:transparent;">…</span>';
      } else {
        html +=
          '<button class="leb-pm-page-btn' +
          (p === page ? " leb-pm-page-btn--active" : "") +
          '" data-page="' +
          p +
          '">' +
          p +
  ...
  function updateTabCounts(counts) {
    if (DOM.countPublished)
      DOM.countPublished.textContent = counts.published || 0;
  ...
  function bindEvents() {
    ...
    if (DOM.searchInput) {
      ...
      DOM.searchInput.addEventListener("focus", function () {
        if (DOM.searchWrapper)
          DOM.searchWrapper.classList.add("leb-pm-search-focused");
      });

      DOM.searchInput.addEventListener("blur", function () {
        if (DOM.searchWrapper)
          DOM.searchWrapper.classList.remove("leb-pm-search-focused");
      });
    }
    ...
    if (DOM.body) {
      ...
      // ── Single Delete ─────────────────────────────────
      // ── Single Actions (Delete/Duplicate) ────────────────
      DOM.body.addEventListener("click", function (e) {
        const delBtn = e.target.closest(".leb-pm-delete-btn");
        const dupBtn = e.target.closest(".leb-pm-duplicate-btn");
        ...
        if (dupBtn) {
          const id = parseInt(dupBtn.dataset.id, 10);
          if (!id) return;
          performAJAXAction(
            "leb_listing_duplicate",
            [id],
      ...
      onConfirm: function () {
        performAJAXAction(
          ...
          function () {
            ...
            if (
              actionType === "delete" &&
              DOM.body.children.length <= ids.length &&
              lebPmState.page > 1
            ) {
              lebPmState.page--;
            }
  ...
  function performAJAXAction(action, ids, successMsg, onSuccess, newStatus) {
    ...
    jQuery
      .post(LEB_Ajax.ajax_url, data, function (res) {
        if (res.success) {
          LEB_Toaster.show(successMsg, "success");
          if (typeof onSuccess === "function") onSuccess();
        } else {
          LEB_Toaster.show(
            (res.data && res.data.message) || "Action failed.",
            "error",
          );
        }
      })
